...
We only introduce the unit type, since the product type is a special case of the
dependent pair type.
*)
...
(** Version of double using the induction principle instead of the induction tactic. *)
Definition double'(n: nat): nat
...
Notation "¬ A" := (neg A) (at level 35, right associativity).
Notation "¬¬ A" := (¬ (¬ A)) (at level 35, right associativity).
...
Goal ∏ (A: UU) (P Q: A → UU), (∏ (x: A), P x × Q x) → (∏ (x: A), P x) × (∏ (x: A), Q x).
Proof.
  intros A P Q H.
  apply tpair.
  - exact (λ x, pr1 (H x)).
...
(* allow to use idpath in the reflexivity tactic*)
...
Lemma paths_rect_free {A: UU} (C: ∏ x y: A, x = y → UU) (c: ∏ x: A, C x x (idpath x))
  : ∏ (x y: A) (p: x = y), C x y p.
Proof.
  intro.
...
Lemma paths_rect_free_simpl {A : UU} (C : ∏ x y : A, x = y → UU)
  (c: ∏ x: A, C x x (idpath x)) (x: A)
  : paths_rect_free C c x x (idpath x) = c x.
Proof.
  reflexivity.
Defined.
...
Lemma paths_free_space (A: UU) (x y: A) (p: paths_free x y)
  : @paths_free (∑ (x y : A), paths_free x y) (x,, y,, p) (x,, x,, idpath' x).
Proof.
...
Proof.
  ...
  revert C c.
  pose (P := λ (x y: A) (p: paths_free x y),
    ∏ (C: ∏ (y0: A), paths_free x y0 → UU), C x (idpath' x) → C y p).
  apply (paths_free_rect A P).
  unfold P.
  intros.
  assumption.
...
Lemma paths_free_rect_based_simpl {A: UU} (x: A) (C: ∏ y: A, paths_free x y → UU)
  (c: C x (idpath' x))
  : paths_free_rect_based x C c x (idpath' x) = c.
Proof.
  reflexivity.
Defined.
...
Notation "g ∘ f" := (funcomp g f) (at level 40, left associativity) : function_scope.
...
Local Definition sum_myrec {A: UU} {B: A → UU} {C: UU} (g: ∏ (a: A) (b: B a), C)
...
Local Theorem nat_myrecaux_compsucc {C: UU} (c0: C) (cs: nat → C → C) (n: nat)
  : nat_myrecaux c0 cs (S n) = S (pr1 (nat_myrecaux c0 cs n)),, cs (pr1 (nat_myrecaux c0 cs n)) (pr2 (nat_myrecaux c0 cs n)).
Proof.
  reflexivity.
Defined.
...
Local Definition pair' {A B: UU} (a: A) (b: B): prod' A B := bool_rect (bool_rect (λ _, UU) A B) a b.
